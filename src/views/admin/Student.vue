<template>
    <!-- 教师列表 -->
    <div v-if="adminStore.isShowStudentList == true">
    <!-- 搜索区 -->
    <div class="search">
        <div class="search-title">
            <div class="time">
                ...
                <div>
                    <span>学生名字</span>
                    <el-input v-model="inputStudentName" style="width: 180px" placeholder="请输入学生名字" />
                </div>
                <div>
                    <span>账号</span>
                    <el-input v-model="inputStudentAccount" style="width: 180px" placeholder="请输入账号" />
                </div>
            </div>
    ...
    <div class="table">
        ...
        <div class="table-title">
            ...
            <div class="new" >
                <div @click="handleClickNew()">+&nbsp;创建学生账号</div>
                <div style="display: flex;align-items: flex-start;gap: 10px">
                    <el-button type="primary" @click="handleClickMultiple()">批量创建</el-button>
                    <el-tooltip placement="top" effect="light" content="一次创建多个学生账号">
...
import { fetchAllStudent } from '../../apis/modules/user'
...
const tableList: Ref<TACH_LIST[]> = ref([]);    //学生列表信息
const tableListAll = ref(); //暂存所有学生列表
...
const handleSearch = () => {
    const filteredList = tableListAll.value.filter((teacher: any) => {
        ...
        const nameMatched = inputStudentName.value === null || teacher.name.includes(inputStudentName.value);
        const accountMatched = inputStudentAccount.value === null || teacher.account.includes(inputStudentAccount.value);
...
const handleClickNew = () => {
    adminStore.setIsShowStudentList(false);
    adminStore.setIsEditStudent(false)
}
...
const handleEdit = (row: any) => {
    adminStore.setIsShowStudentList(false);
    adminStore.setIsEditStudent(true);
    //保存当前学生信息
    adminStore.setActiveStudentItem(row);
}
