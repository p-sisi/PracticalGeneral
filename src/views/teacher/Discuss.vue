        ...
        <div class="discuss-list">
            <div class="item" v-for="item in discussListData" :key="item.id" @click="handleClickItem(item)">
                <div style="display: flex;gap: 20px;align-items: center;">
                    <div> 
                        <div v-if="item.isTop == true" class="item-top">置顶</div>
                ...
                <div class="item-btn">
                    <el-dropdown @command="(command: any) => handleCommand(item, command)">
                        ...
                        <template #dropdown>
                            <el-dropdown-menu>
                                <el-dropdown-item command="top">{{item.isTop == false? '设为置顶': '取消置顶'}}</el-dropdown-item>
...
import { Search, Plus, ArrowLeftBold, ArrowDown, ArrowUp, Position, InfoFilled, Delete, MoreFilled } from '@element-plus/icons-vue'
import type { FormRules, FormInstance } from 'element-plus';
import { ElMessage } from 'element-plus'
import { Discuss, Discuss_First_Reply } from '../../content/discuss'
import { BASE_ERL } from '../../content/common';
import { fetchDiscussByCourseId, fetchDiscussComment, fetchNewDiscuss, fetchReplyDiscuss, fetchDeleteReplyDiscuss, fetchDeleteDiscuss, fetchTopDiscuss, fetchCancelTopDiscuss } from '../../apis/modules/discuss';
 ...
 *  置顶讨论请求封装
*/
const setTopDiscussRequest = async(id: number) => {
    try {
        const params = {
            discussionId: id
        }
        await fetchTopDiscuss(params);
 ...
 *  取消置顶讨论请求封装
*/
const cancelTopDiscussRequest = async(id: number) => {
    try {
        const params = {
            discussionId: id
        }
        await fetchCancelTopDiscuss(params);
...
const handleCommand = (item: any, command: string | number | object) => {
    if(command == 'delete') {
        handleDeleteDiscuss(item);
    }else {
        if(item.isTop == false) {
            setTopDiscussRequest(item.id)
        }else{
            cancelTopDiscussRequest(item.id)
        }
    }
}
